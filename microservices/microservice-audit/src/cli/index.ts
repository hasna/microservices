...
import { countEvents, exportEvents, queryEvents } from "../lib/events.js";
...
program
  ...
  .description(
    "Audit microservice — immutable event log, tamper-evident checksums, retention policies",
  )
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'audit'`;
      console.log(
        "  Schema 'audit':",
      ...
      if (schemas.length > 0) {
        const [{ count }] =
          await sql`SELECT COUNT(*) as count FROM audit.events`;
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (opts.json) {
        console.log(
          JSON.stringify({ data: events, count: events.length }, null, 2),
        );
        return;
      }
      if (events.length === 0) {
      ...
      for (const e of events) {
        const ts =
          e.created_at instanceof Date
            ...
            : String(e.created_at);
        console.log(
          `  [${ts}] [${e.severity}] ${e.action} on ${e.resource_type}${e.resource_id ? `/${e.resource_id}` : ""} by ${e.actor_id ?? "system"}`,
        );
      }
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log(
        "✓ microservice-audit ready\n  Schema: audit.*\n  Next: microservice-audit serve --port 3006",
      );
    } finally {
      await closeDb();
    }
    ...
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        check(
          "Schema 'audit' exists",
          schemas.length > 0,
          "run: microservice-audit migrate",
        );
        if (schemas.length > 0) {
          const [{ count }] =
            await sql`SELECT COUNT(*) as count FROM audit.events`;
