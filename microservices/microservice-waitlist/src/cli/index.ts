...
import { inviteBatch, listEntries } from "../lib/entries.js";
...
program
  ...
  .description(
    "Waitlist microservice — campaign management, referral tracking, priority scoring",
  )
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'waitlist'`;
      console.log(
        "  Schema 'waitlist':",
      ...
      if (schemas.length > 0) {
        const [{ count }] =
          await sql`SELECT COUNT(*) as count FROM waitlist.campaigns`;
        console.log("  Campaigns:", count);
        const [{ entries }] =
          await sql`SELECT COUNT(*) as entries FROM waitlist.entries`;
    ...
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        const schemas =
          await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'waitlist'`;
        check(
          "Schema 'waitlist' exists",
          schemas.length > 0,
          "run: microservice-waitlist migrate",
        );
        if (schemas.length > 0) {
          const [{ count }] =
            await sql`SELECT COUNT(*) as count FROM waitlist.campaigns`;
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (opts.json) {
        console.log(
          JSON.stringify({ data: invited, count: invited.length }, null, 2),
        );
        return;
      }
...
program
  ...
  .option(
    "--status <status>",
    "Filter by status: waiting|invited|joined|removed",
  )
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (opts.json) {
        console.log(
          JSON.stringify({ data: entries, count: entries.length }, null, 2),
        );
        return;
      }
      if (entries.length === 0) {
      ...
      for (const e of entries) {
        const ts =
          e.created_at instanceof Date
            ...
            : String(e.created_at);
        console.log(
          `  [${ts}] [${e.status}] ${e.email}${e.name ? ` (${e.name})` : ""} score=${e.priority_score} refs=${e.referral_count}`,
        );
      }
