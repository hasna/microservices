...
import { scanPII, redactPII } from "../lib/pii.js";
...
program
  ...
  .description("Guardrails microservice — PII detection, prompt injection defense, toxicity filtering, policy enforcement")
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'guardrails'`;
      console.log("  Schema 'guardrails':", schemas.length > 0 ? "✓ exists" : "✗ missing (run migrate)");
      if (schemas.length > 0) {
        const [{ count: pCount }] = await sql`SELECT COUNT(*) as count FROM guardrails.policies`;
        const [{ count: vCount }] = await sql`SELECT COUNT(*) as count FROM guardrails.violations`;
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      await migrate(sql);
      const result = opts.direction === "output"
      ...
      if (result.violations.length > 0) {
        ...
        for (const v of result.violations) {
          console.log(`    [${v.severity}] ${v.type}: ${JSON.stringify(v.details)}`);
        }
      }
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (opts.json) {
        console.log(JSON.stringify({ data: violations, count: violations.length }, null, 2));
        return;
      }
      ...
      for (const v of violations) {
        const ts = v.created_at instanceof Date ? v.created_at.toISOString() : String(v.created_at);
        console.log(`  [${ts}] [${v.severity}] ${v.type} (${v.direction}) ${v.content_snippet ? v.content_snippet.slice(0, 60) + "..." : ""}`);
      }
  ...
  .action(async (opts) => {
    ...
    if (process.env["DATABASE_URL"]) {
      ...
      try {
        ...
        const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'guardrails'`;
        check("Schema 'guardrails' exists", schemas.length > 0, "run: microservice-guardrails migrate");
        if (schemas.length > 0) {
          const [{ count: pCount }] = await sql`SELECT COUNT(*) as count FROM guardrails.policies`;
          const [{ count: vCount }] = await sql`SELECT COUNT(*) as count FROM guardrails.violations`;
