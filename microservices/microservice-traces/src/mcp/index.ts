  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          name: { type: "string", description: "Trace name (e.g. chat, completion)" },
    ...
    },
    {
      ...
      description: "End a trace and compute aggregated metrics from child spans",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          parent_span_id: { type: "string", description: "Parent span ID for nesting" },
          name: { type: "string", description: "Span name" },
          type: { type: "string", enum: ["llm", "tool", "retrieval", "guardrail", "embedding", "custom"], description: "Span type" },
          input: { type: "object", description: "Input data" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          workspace_id: { type: "string" },
          since: { type: "string", description: "ISO date string (default: 30 days ago)" },
        },
        required: ["workspace_id"],
      },
    },
    {
      ...
      description: "Get a trace with spans nested as a tree (children[] on each span)",
      inputSchema: {
        type: "object",
        properties: {
          id: { type: "string", description: "Trace ID" },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (name === "traces_start_trace") {
    return text(await startTrace(sql, {
  ...
  if (name === "traces_end_trace") {
    return text(await endTrace(sql, String(a.id), {
  ...
  if (name === "traces_start_span") {
    return text(await startSpan(sql, {
      traceId: String(a.trace_id),
      parentSpanId: a.parent_span_id ? String(a.parent_span_id) : undefined,
      name: String(a.name),
      type: String(a.type) as "llm" | "tool" | "retrieval" | "guardrail" | "embedding" | "custom",
      input: a.input,
  ...
  if (name === "traces_end_span") {
    return text(await endSpan(sql, String(a.id), {
      status: a.status as "completed" | "error",
      output: a.output,
      error: a.error ? String(a.error) : undefined,
  ...
  if (name === "traces_list_traces") {
    return text(await listTraces(sql, String(a.workspace_id), {
  ...
  if (name === "traces_get_stats") {
    return text(await getTraceStats(sql, String(a.workspace_id), a.since ? new Date(String(a.since)) : undefined));
  }
