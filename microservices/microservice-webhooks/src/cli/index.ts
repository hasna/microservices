...
import { replayDelivery, triggerWebhook } from "../lib/deliver.js";
import { listWorkspaceEndpoints } from "../lib/endpoints.js";

const program = new Command();
program
  .name("microservice-webhooks")
  .description(
    "Webhooks microservice — endpoint registry, event delivery, HMAC signatures, retry",
  )
  .version("0.0.1");
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log("✓ webhooks schema migrations complete");
    } finally {
      await closeDb();
    }
  });
...
program
  ...
  .action(async (o) => {
    ...
    try {
      ...
      const s =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'webhooks'`;
      console.log(
        "  Schema 'webhooks':",
    ...
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        const schemas =
          await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'webhooks'`;
        check(
          "Schema 'webhooks' exists",
          schemas.length > 0,
          "run: microservice-webhooks migrate",
        );
        if (schemas.length > 0) {
          const tables =
            await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'webhooks' ORDER BY table_name`;
          check(
    ...
    try {
      ...
      if (o.json) {
        ...
        for (const ep of endpoints) {
          ...
          console.log(
            `  ${ep.id}  ${ep.url.padEnd(50)} events:${evts}  ${ep.active ? "active" : "disabled"}  failures:${ep.failure_count}`,
          );
        }
      }
    } finally {
  ...
  .action(async (o) => {
    ...
    try {
      await triggerWebhook(
        ...
        o.workspace,
        o.event,
        JSON.parse(o.payloadJson),
      );
      console.log(
        `✓ Triggered event "${o.event}" for workspace ${o.workspace}`,
      );
    } finally {
      await closeDb();
    }
