...
import { getMember, listMembers, removeMember } from "../lib/members.js";
import { createWorkspace, listUserWorkspaces } from "../lib/workspaces.js";

const program = new Command();
program
  .name("microservice-teams")
  .description("Teams microservice — workspaces, RBAC, invites")
  .version("0.0.1");
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log("✓ teams schema migrations complete");
    } finally {
      await closeDb();
    }
  });
...
program
  ...
  .option("--port <n>", "Port", String(process.env.TEAMS_PORT ?? "3002"))
  .option("--db <url>")
  .action(async (o) => {
    if (o.db) process.env.DATABASE_URL = o.db;
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      const ws = await createWorkspace(sql, { name: o.name, ownerId: o.owner });
      console.log("✓ Created:", ws.id, ws.name, ws.slug);
    } finally {
      await closeDb();
    }
  });
...
program
  ...
  .description("List workspaces for a user")
  ...
  .action(async (o) => {
    ...
    const sql = getDb();
    try {
      const wss = await listUserWorkspaces(sql, o.user);
      if (o.json) {
        console.log(JSON.stringify(wss, null, 2));
      } else {
        for (const w of wss) console.log(`  ${w.id}  ${w.slug}  ${w.name}`);
      }
    } finally {
      await closeDb();
    }
...
program
  ...
  .action(async (id, o) => {
    ...
    const sql = getDb();
    try {
      const ms = await listMembers(sql, id);
      if (o.json) {
        console.log(JSON.stringify(ms, null, 2));
      } else {
        for (const m of ms) console.log(`  ${m.user_id}  ${m.role}`);
      }
    } finally {
      await closeDb();
    }
...
program
  ...
  .action(async (o) => {
    ...
    try {
      ...
      const s =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'teams'`;
      console.log("  Schema 'teams':", s.length > 0 ? "✓" : "✗ (run migrate)");
    } finally {
      await closeDb();
    }
  });
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log(
        "✓ microservice-teams ready\n  Schema: teams.*\n  Next: microservice-teams serve --port 3002",
      );
    } finally {
      await closeDb();
    }
    ...
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        check(
          "Schema 'teams' exists",
          schemas.length > 0,
          "run: microservice-teams migrate",
        );
        if (schemas.length > 0) {
          const tables =
            await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'teams' ORDER BY table_name`;
          check(
...
program
  ...
  .description("Leave a workspace (non-owners only)")
  ...
  .action(async (o) => {
    ...
    try {
      ...
      if (member.role === "owner") {
        console.error("✗ Owners cannot leave — transfer ownership first");
        process.exit(1);
      }
