...
import { listMembers, addMember, updateMemberRole, removeMember, checkPermission, transferOwnership, getMember } from "../lib/members.js";
    ...
    try {
      if (m === "GET" && p === "/health") {
        try {
          ...
          await sql`SELECT 1`;
          return json({ ok: true, service: "microservice-teams", db: true, latency_ms: Date.now() - start });
        } catch (e) {
          return json({ ok: false, service: "microservice-teams", db: false, error: e instanceof Error ? e.message : "db error" }, 503);
        }
      }
      // Workspaces
      if (m === "POST" && p === "/teams/workspaces") {
        ...
        return json(await createWorkspace(sql, { name: parsed.data.name, ownerId: parsed.data.owner_id, slug: parsed.data.slug }), 201);
      }
      if (m === "GET" && p.match(/^\/teams\/workspaces\/[^/]+$/)) {
        const id = p.split("/").pop()!;
        const ws = await getWorkspace(sql, id);
        return ws ? json(ws) : apiError("NOT_FOUND", "Not found", undefined, 404);
      }
      if (m === "GET" && p === "/teams/workspaces" && url.searchParams.get("user_id")) {
        const workspaces = await listUserWorkspaces(sql, url.searchParams.get("user_id")!);
      ...
      if (m === "POST" && p.match(/^\/teams\/workspaces\/[^/]+\/members$/)) {
        ...
        if ("error" in parsed) return parsed.error;
        return json(await addMember(sql, wsId, parsed.data.user_id, parsed.data.role), 201);
      }
      if (m === "PATCH" && p.match(/^\/teams\/workspaces\/[^/]+\/members\/[^/]+$/)) {
        ...
        if ("error" in parsed) return parsed.error;
        const member = await updateMemberRole(sql, wsId, userId, parsed.data.role);
        return member ? json(member) : apiError("NOT_FOUND", "Not found", undefined, 404);
      }
      ...
      if (m === "POST" && p.match(/^\/teams\/workspaces\/[^/]+\/invites$/)) {
        ...
        if ("error" in parsed) return parsed.error;
        return json(await createInvite(sql, { workspaceId: wsId, email: parsed.data.email, role: parsed.data.role, invitedBy: parsed.data.invited_by }), 201);
      }
      ...
      if (m === "POST" && p === "/teams/invites/accept") {
        ...
        if ("error" in parsed) return parsed.error;
        const result = await acceptInvite(sql, parsed.data.token, parsed.data.user_id);
        return result ? json(result) : apiError("INVALID_INVITE", "Invalid or expired invite", undefined, 400);
      }
      ...
      if (m === "POST" && p.match(/^\/teams\/workspaces\/[^/]+\/transfer$/)) {
        ...
        if (!ws) return apiError("NOT_FOUND", "Workspace not found", undefined, 404);
        if (ws.owner_id !== parsed.data.current_owner_id) return apiError("FORBIDDEN", "Requester is not the owner", undefined, 403);
        await transferOwnership(sql, wsId, parsed.data.current_owner_id, parsed.data.new_owner_id);
        return json({ ok: true });
      }
      ...
      if (m === "POST" && p.match(/^\/teams\/workspaces\/[^/]+\/leave$/)) {
        ...
        if (member.role === "owner") return apiError("FORBIDDEN", "Owners cannot leave — transfer ownership first", undefined, 403);
        await removeMember(sql, wsId, parsed.data.user_id);
        return json({ ok: true });
      }
      // Bulk invite
      if (m === "POST" && p.match(/^\/teams\/workspaces\/[^/]+\/invites\/bulk$/)) {
        const wsId = p.split("/")[3];
        ...
        for (const email of parsed.data.emails) {
          try {
            const invite = await createInvite(sql, { workspaceId: wsId, email, role: parsed.data.role, invitedBy: parsed.data.invited_by });
            results.push({ email, success: true, token: invite.token });
          } catch (e) {
            results.push({ email, success: false, error: e instanceof Error ? e.message : "Failed to create invite" });
          }
        }
        return json({ results });
      }
