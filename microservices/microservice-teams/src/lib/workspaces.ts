...
export async function createWorkspace(
  sql: Sql,
  data: { name: string; ownerId: string; slug?: string },
): Promise<Workspace> {
  const slug =
    data.slug ?? `${slugify(data.name)}-${crypto.randomUUID().slice(0, 8)}`;
...
export async function getWorkspace(
  ...
  const [ws] = await sql<
    Workspace[]
  >`SELECT * FROM teams.workspaces WHERE id = ${id}`;
  return ws ?? null;
}
...
export async function getWorkspaceBySlug(
  ...
  const [ws] = await sql<
    Workspace[]
  >`SELECT * FROM teams.workspaces WHERE slug = ${slug}`;
  return ws ?? null;
}
