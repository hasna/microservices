...
function getDbPath(): string {
  ...
  if (explicit) {
    return join(explicit, "microservice-proposals", "data.db");
  }

  let dir = resolve(process.cwd());
  while (true) {
    const msDir = join(dir, ".microservices");
    if (existsSync(msDir)) return join(msDir, "microservice-proposals", "data.db");
  ...
  return join(newDir, "microservice-proposals", "data.db");
}
