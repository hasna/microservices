...
function getDbPath(): string {
  ...
  if (explicit) {
    return join(explicit, "microservice-payroll", "data.db");
  }

  let dir = resolve(process.cwd());
  while (true) {
    const msDir = join(dir, ".microservices");
    if (existsSync(msDir)) return join(msDir, "microservice-payroll", "data.db");
  ...
  return join(newDir, "microservice-payroll", "data.db");
}
