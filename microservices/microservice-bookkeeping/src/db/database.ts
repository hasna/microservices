...
function getDbPath(): string {
  ...
  if (explicit) {
    return join(explicit, "microservice-bookkeeping", "data.db");
  }

  let dir = resolve(process.cwd());
  while (true) {
    const msDir = join(dir, ".microservices");
    if (existsSync(msDir)) return join(msDir, "microservice-bookkeeping", "data.db");
  ...
  return join(newDir, "microservice-bookkeeping", "data.db");
}
