...
function getDbPath(): string {
  ...
  if (explicit) {
    return join(explicit, "microservice-shipping", "data.db");
  }

  let dir = resolve(process.cwd());
  while (true) {
    const msDir = join(dir, ".microservices");
    if (existsSync(msDir)) return join(msDir, "microservice-shipping", "data.db");
  ...
  return join(newDir, "microservice-shipping", "data.db");
}
