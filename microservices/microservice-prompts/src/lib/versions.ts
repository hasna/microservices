...
export async function updatePrompt(
  sql: Sql,
  promptId: string,
  opts: { content: string; changeNote?: string; createdBy?: string; model?: string }
): Promise<Version> {
  ...
  return await sql.begin(async tx => {
    ...
    await tx`UPDATE prompts.prompts SET current_version_id = ${version.id}, updated_at = NOW() WHERE id = ${promptId}`;
    return version as unknown as Version;
  });
}

export async function getVersion(sql: Sql, promptId: string, versionNumber?: number): Promise<Version | null> {
...
export async function listVersions(sql: Sql, promptId: string): Promise<Version[]> {
  return (await sql`
    SELECT * FROM prompts.versions WHERE prompt_id = ${promptId} ORDER BY version_number DESC`) as unknown as Version[];
}

export async function rollback(sql: Sql, promptId: string, toVersionNumber: number): Promise<void> {
  const [version] = await sql`
    SELECT id FROM prompts.versions WHERE prompt_id = ${promptId} AND version_number = ${toVersionNumber}`;
  if (!version) throw new Error(`Version ${toVersionNumber} not found for prompt ${promptId}`);
  await sql`UPDATE prompts.prompts SET current_version_id = ${version.id}, updated_at = NOW() WHERE id = ${promptId}`;
}

export function diffVersions(_promptId: string, v1Content: string, v2Content: string): DiffResult {
