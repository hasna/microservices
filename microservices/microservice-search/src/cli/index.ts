...
program
  ...
  .description("Search microservice — full-text, semantic (pgvector), and hybrid document search")
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'search'`;
      console.log("  Schema 'search':", schemas.length > 0 ? "✓ exists" : "✗ missing (run migrate)");
      if (schemas.length > 0) {
...
program
  .command("doctor")
  .description("Check configuration, env vars, DB connectivity, and optional features")
  .option("--db <url>", "PostgreSQL connection URL")
  .action(async (opts) => {
    if (opts.db) process.env["DATABASE_URL"] = opts.db;
    let ok = true;
    ...
    if (process.env["DATABASE_URL"]) {
      ...
      try {
        ...
        const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'search'`;
        check("Schema 'search' exists", schemas.length > 0, "run: microservice-search migrate");

        if (schemas.length > 0) {
          const tables = await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'search' ORDER BY table_name`;
          const names = tables.map((t: { table_name: string }) => t.table_name).join(", ");
          check(`Tables (${tables.length})`, tables.length >= 1, `found: ${names}`);
          ...
          try {
            ...
            `;
            check("pgvector column (embedding)", has_vector, "run migrate, or install the vector extension");
          } catch {
            check("pgvector column (embedding)", false, "extension not available");
          }
        }
      } catch (e) {
    ...
    try {
      ...
      if (opts.json) {
        ...
        for (const r of results) {
          console.log(`  [${r.collection}] ${r.doc_id}  score=${r.score.toFixed(4)}`);
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      console.log(`✓ Indexed document '${opts.docId}' in collection '${opts.collection}'`);
    } finally { await closeDb(); }
  });
