...
describe("findAgentByCapability — preference logic", () => {
  const agents = [
    { id: "a1", status: "active", capabilities: ["code", "search"], current_load: 2, max_concurrent: 3 },
    { id: "a2", status: "idle", capabilities: ["code", "deploy"], current_load: 0, max_concurrent: 2 },
    { id: "a3", status: "active", capabilities: ["search"], current_load: 1, max_concurrent: 1 },
    { id: "a4", status: "stopped", capabilities: ["code"], current_load: 0, max_concurrent: 1 },
  ];
  ...
  function findByCapability(capability: string, preferIdle = false) {
    ...
    if (preferIdle) {
      available.sort((a, b) => {
        ...
        return statusOrder(a.status) - statusOrder(b.status) || a.current_load - b.current_load;
      });
    } else {
      available.sort((a, b) => {
  ...
  it("agent at max_concurrent is not selected", () => {
    ...
    const searchAgents = agents.filter(a =>
      a.capabilities.includes("search") && ["active", "idle"].includes(a.status)
    );
    expect(searchAgents).toHaveLength(2); // a1 and a3
    const available = searchAgents.filter(a => a.current_load < a.max_concurrent);
...
describe("task priority: higher priority tasks claimed first", () => {
  it("sorts tasks by priority descending", () => {
    ...
    const sorted = [...tasks].sort((a, b) => b.priority - a.priority || new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
...
describe("health report counts are correct", () => {
  it("correctly counts agents by status", () => {
    ...
    expect(report.active + report.idle + report.stopped + report.error).toBe(report.total);
  });
});
