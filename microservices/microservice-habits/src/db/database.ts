...
function getDbPath(): string {
  ...
  if (explicit) {
    return join(explicit, "microservice-habits", "data.db");
  }

  let dir = resolve(process.cwd());
  while (true) {
    const msDir = join(dir, ".microservices");
    if (existsSync(msDir)) return join(msDir, "microservice-habits", "data.db");
  ...
  return join(newDir, "microservice-habits", "data.db");
}
