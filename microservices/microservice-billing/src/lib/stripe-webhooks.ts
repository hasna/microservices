  ...
  switch (event.type) {
    ...
    case "customer.subscription.updated": {
      ...
      const canceledAt = obj.canceled_at
        ? new Date(Number(obj.canceled_at) * 1000).toISOString()
        : undefined;

      // Look up existing subscription by stripe id to preserve workspace_id/user_id/plan_id
      const existing =
        await sql`SELECT * FROM billing.subscriptions WHERE stripe_subscription_id = ${stripeSubId} LIMIT 1`;
      if (existing.length > 0) {
        ...
        const metaWorkspaceId = String(
          (obj.metadata as any | undefined)?.workspace_id ?? "",
        );
        const metaUserId = String(
          (obj.metadata as any | undefined)?.user_id ?? "",
        );
        const metaPlanId = String(
          (obj.metadata as any | undefined)?.plan_id ?? "",
        );
    ...
    case "customer.subscription.deleted": {
      const stripeSubId = String(obj.id ?? "");
      const rows =
        await sql`SELECT id FROM billing.subscriptions WHERE stripe_subscription_id = ${stripeSubId} LIMIT 1`;
      if (rows.length > 0) {
    ...
    case "invoice.payment_failed": {
      ...
      const stripeSubId = obj.subscription
        ...
        : undefined;
      const invoicePdfUrl = obj.invoice_pdf
        ? String(obj.invoice_pdf)
        : undefined;
      const dueDate = obj.due_date
        ? new Date(Number(obj.due_date) * 1000).toISOString()
        : undefined;
      ...
      if (stripeSubId) {
        const subRows =
          await sql`SELECT id, workspace_id FROM billing.subscriptions WHERE stripe_subscription_id = ${stripeSubId} LIMIT 1`;
        if (subRows.length > 0) {
          const subRow = subRows[0] as any;
      ...
      if (!workspaceId) {
        // Try metadata
        workspaceId = String(
          (obj.metadata as any | undefined)?.workspace_id ?? "unknown",
        );
      }
