...
import { validatePlanData, VALID_CURRENCIES, VALID_INTERVALS } from "./plans.js";

describe("validatePlanData", () => {
  it("should pass for a valid monthly plan", () => {
    const errors = validatePlanData({ name: "Pro Plan", amount_cents: 999, currency: "usd", interval: "month" });
  ...
  it("should fail for an invalid currency", () => {
    const errors = validatePlanData({ name: "Plan", amount_cents: 100, currency: "xyz" });
    expect(errors.some(e => e.includes("currency"))).toBe(true);
  });

  it("should fail for an invalid interval", () => {
    const errors = validatePlanData({ name: "Plan", amount_cents: 100, interval: "weekly" as any });
  ...
  it("should pass for a yearly plan with valid currency", () => {
    const errors = validatePlanData({ name: "Annual", amount_cents: 9900, currency: "eur", interval: "year" });
    expect(errors).toHaveLength(0);
  });

  it("should pass for a one_time payment plan", () => {
    const errors = validatePlanData({ name: "Lifetime", amount_cents: 29900, interval: "one_time" });
    expect(errors).toHaveLength(0);
  });
