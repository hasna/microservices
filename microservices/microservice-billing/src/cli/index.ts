...
program
  ...
  .description("Billing microservice — plans, subscriptions, invoices, Stripe")
  .version("0.0.1");
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log("✓ billing schema migrations complete");
    } finally {
      await closeDb();
    }
  });
...
program
  ...
  .option("--port <n>", "Port", String(process.env.BILLING_PORT ?? "3003"))
  .option("--db <url>")
  .action(async (o) => {
    if (o.db) process.env.DATABASE_URL = o.db;
...
program
  ...
  .action(async (o) => {
    ...
    try {
      ...
      const s =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'billing'`;
      console.log(
        "  Schema 'billing':",
        s.length > 0 ? "✓" : "✗ (run migrate)",
      );
    } finally {
      await closeDb();
    }
...
program
  ...
  .option("--db <url>")
  .option("--active", "Only active plans")
  ...
  .action(async (o) => {
    ...
    try {
      ...
      for (const p of plans)
        console.log(
          `  ${p.id}  ${p.name}  ${p.amount_cents / 100} ${p.currency.toUpperCase()}/${p.interval}  active=${p.active}`,
        );
    } finally {
      await closeDb();
    }
...
program
  ...
  .action(async (o) => {
    ...
    const sql = getDb();
    try {
      const subs = await listSubscriptions(sql, {
      ...
      for (const s of subs)
        console.log(
          `  ${s.id}  workspace=${s.workspace_id}  status=${s.status}  plan=${s.plan_id}`,
        );
    } finally {
      await closeDb();
    }
...
program
  ...
  .argument("<subscription-id>", "Subscription ID to cancel")
  .option("--immediately", "Cancel immediately instead of at period end")
  .option("--db <url>")
  .action(async (subscriptionId, o) => {
    if (o.db) process.env.DATABASE_URL = o.db;
    const sql = getDb();
    try {
      ...
      console.log(
        `✓ Subscription ${subscriptionId} canceled  status=${updated?.status}  cancel_at_period_end=${updated?.cancel_at_period_end}`,
      );
    } finally {
      await closeDb();
    }
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log(
        "✓ microservice-billing ready\n  Schema: billing.*\n  Requires: STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET\n  Next: microservice-billing serve --port 3003",
      );
    } finally {
      await closeDb();
    }
    ...
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        const schemas =
          await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'billing'`;
        check(
          "Schema 'billing' exists",
          schemas.length > 0,
          "run: microservice-billing migrate",
        );
      } catch (e) {
        check(
