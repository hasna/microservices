...
export function makeRouter(sql: Sql) {
  return async function router(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (
        m === "GET" &&
        p.match(/^\/billing\/workspaces\/[^/]+\/subscription$/)
      ) {
        const workspaceId = p.split("/")[3];
        const sub = await getWorkspaceSubscription(sql, workspaceId);
        return sub
          ? json(sub)
          : apiError("NOT_FOUND", "No active subscription", undefined, 404);
      }
      ...
      if (m === "POST" && p === "/billing/checkout") {
        ...
        const stripeKey =
          rawBody.stripe_secret_key ?? process.env.STRIPE_SECRET_KEY ?? "";
        if (!stripeKey)
          return apiError("MISSING_CONFIG", "STRIPE_SECRET_KEY required");
        const result = await createCheckoutSession({
          ...
          trialPeriodDays: rawBody.trial_period_days
            ? parseInt(rawBody.trial_period_days, 10)
            : undefined,
        });
        return json(result);
      }
      ...
      if (m === "POST" && p === "/billing/webhooks") {
        ...
        try {
          ...
          if (err instanceof WebhookSignatureError)
            return apiError("WEBHOOK_SIGNATURE_INVALID", err.message);
          throw err;
        }
      }
