...
function getDbPath(): string {
  ...
  if (process.env["MICROSERVICES_DIR"]) {
    return join(process.env["MICROSERVICES_DIR"], "microservice-contacts", "data.db");
  }

  // Check for .microservices in current or parent directories
  let dir = resolve(process.cwd());
  while (true) {
    const candidate = join(dir, ".microservices", "microservice-contacts", "data.db");
  ...
  return join(home, ".microservices", "microservice-contacts", "data.db");
}
