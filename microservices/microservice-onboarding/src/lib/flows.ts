...
export async function getFlow(sql: Sql, id: string): Promise<Flow | null> {
  const [row] = await sql<
    Flow[]
  >`SELECT * FROM onboarding.flows WHERE id = ${id}`;
  return row ?? null;
}

export async function getFlowByName(
  sql: Sql,
  name: string,
): Promise<Flow | null> {
  const [row] = await sql<
    Flow[]
  >`SELECT * FROM onboarding.flows WHERE name = ${name}`;
  return row ?? null;
}
...
export async function listFlows(sql: Sql, activeOnly = false): Promise<Flow[]> {
  if (activeOnly) {
    return sql<
      Flow[]
    >`SELECT * FROM onboarding.flows WHERE active = true ORDER BY created_at ASC`;
  }
...
export async function updateFlow(
  ...
  if (data.steps !== undefined) {
    sets.push(`steps = $${values.length + 1}::jsonb`);
