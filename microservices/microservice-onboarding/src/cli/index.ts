...
program
  ...
  .description("Onboarding microservice — flows, steps, and user progress tracking")
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'onboarding'`;
      console.log("  Schema 'onboarding':", schemas.length > 0 ? "✓ exists" : "✗ missing (run migrate)");
      if (schemas.length > 0) {
        const [{ count }] = await sql`SELECT COUNT(*) as count FROM onboarding.flows`;
        console.log("  Flows:", count);
        const [{ count: progressCount }] = await sql`SELECT COUNT(*) as count FROM onboarding.progress`;
  ...
  .action(async (opts) => {
    ...
    if (process.env["DATABASE_URL"]) {
      ...
      try {
        ...
        const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'onboarding'`;
        check("Schema 'onboarding' exists", schemas.length > 0, "run: microservice-onboarding migrate");
        if (schemas.length > 0) {
          const tables = await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'onboarding' ORDER BY table_name`;
    ...
    try {
      ...
      if (opts.json) {
        ...
        for (const f of flows) {
          ...
          console.log(`  ${f.id}  ${f.name}  ${f.active ? "active" : "inactive"}  (${stepsCount} steps)`);
        }
      }
    } finally { await closeDb(); }
  });
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const flow = await createFlow(sql, { name: opts.name, description: opts.description, steps });
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (opts.json) {
        ...
        console.log(`Progress: ${summary.percentage}%  ${summary.is_complete ? "✓ complete" : "in progress"}`);
        console.log(`Completed steps (${summary.completed_steps.length}): ${summary.completed_steps.join(", ") || "none"}`);
        console.log(`Pending steps (${summary.pending_steps.length}): ${summary.pending_steps.map(s => `${s.id}${s.required ? "" : " (optional)"}`).join(", ") || "none"}`);
      }
    } finally { await closeDb(); }
  });
