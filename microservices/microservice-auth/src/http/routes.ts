  ...
  return async function router(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (method === "POST" && path === "/auth/magic-link") {
        ...
        let user = (await getUserByEmail(sql, parsed.data.email)) as any;
      ...
      if (method === "GET" && path === "/auth/me") {
        ...
        return user
          ? json(user)
          : apiError("NOT_FOUND", "User not found", undefined, 404);
      }
      ...
      if (method === "POST" && path === "/auth/change-password") {
        ...
        if ("error" in parsed) return parsed.error;
        const { getUserByEmail, getUserById: getById } = await import(
          "../lib/users.js"
        );
        const user = (await getById(sql, userId)) as any;
        if (!user)
          return apiError("NOT_FOUND", "User not found", undefined, 404);
        const full = await sql<
          [{ password_hash: string | null }]
        >`SELECT password_hash FROM auth.users WHERE id = ${userId}`;
        if (!full[0]?.password_hash)
          return apiError(
            "BAD_REQUEST",
            "Account uses passwordless auth",
            undefined,
            400,
          );
      ...
      const status =
        msg.includes("Invalid") || msg.includes("expired") ? 401 : 500;
...
function getOAuthRedirectUri(provider: string): string {
  ...
  const base =
    process.env.OAUTH_REDIRECT_URI?.replace(":provider", provider) ??
    `http://localhost:${port}/auth/oauth/${provider}/callback`;
  return base;
}

function handleOAuthRedirect(_req: Request, provider: string): Response {
...
async function handleOAuthCallback(
  ...
  let _avatarUrl: string | undefined;
  ...
  if (provider === "github") {
    ...
    if (!tokenData.access_token)
      return apiError(
        "OAUTH_ERROR",
        tokenData.error ?? "Failed to get access token",
      );
    ...
    _avatarUrl = githubUser.avatar_url;
  ...
  const safeUser = resolvedUser as any;
