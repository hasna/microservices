...
program
  ...
  .description("LLM gateway microservice — multi-provider chat, usage tracking, cost calculation")
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'llm'`;
      console.log("  Schema 'llm':", schemas.length > 0 ? "✓ exists" : "✗ missing (run migrate)");
      if (schemas.length > 0) {
        const [{ count }] = await sql`SELECT COUNT(*) as count FROM llm.requests`;
      ...
      if (opts.json) {
        ...
        if (usage.by_model.length > 0) {
          ...
          for (const m of usage.by_model) {
            console.log(`  ${m.model} (${m.provider}): ${m.requests} requests, ${m.total_tokens} tokens, $${m.cost_usd.toFixed(6)}`);
          }
        }
      }
    } finally { await closeDb(); }
...
program
  ...
  .action(() => {
    ...
    if (models.length === 0) {
      console.log("No providers configured. Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or GROQ_API_KEY.");
    } else {
...
program
  ...
  .action(async (opts) => {
    ...
    const hasAnyProvider = !!(process.env["OPENAI_API_KEY"] || process.env["ANTHROPIC_API_KEY"] || process.env["GROQ_API_KEY"]);
    check("At least one provider key", hasAnyProvider, "set OPENAI_API_KEY, ANTHROPIC_API_KEY, or GROQ_API_KEY");
    ...
    if (process.env["DATABASE_URL"]) {
      ...
      try {
        ...
        const schemas = await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'llm'`;
        check("Schema 'llm' exists", schemas.length > 0, "run: microservice-llm migrate");
        if (schemas.length > 0) {
          const tables = await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'llm' ORDER BY table_name`;
