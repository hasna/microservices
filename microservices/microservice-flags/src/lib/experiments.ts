...
export async function getExperiment(
  ...
  const [e] = await sql<
    Experiment[]
  >`SELECT * FROM flags.experiments WHERE id = ${id}`;
  return e ?? null;
}
...
export async function assignVariant(
  ...
  const [existing] = await sql<
    [{ variant: string }]
  >`SELECT variant FROM flags.assignments WHERE experiment_id = ${experimentId} AND user_id = ${userId}`;
...
export async function listExperiments(sql: Sql): Promise<Experiment[]> {
  return sql<
    Experiment[]
  >`SELECT * FROM flags.experiments ORDER BY created_at DESC`;
}
