...
export async function listFlags(
  ...
  return sql<
    Flag[]
  >`SELECT * FROM flags.flags WHERE (workspace_id IS NULL OR workspace_id = ${workspaceId ?? null}) ORDER BY key`;
}
...
export async function removeOverride(
  ...
  const r =
    await sql`DELETE FROM flags.overrides WHERE flag_id = ${flagId} AND target_type = ${targetType} AND target_id = ${targetId}`;
  return r.count > 0;
}
