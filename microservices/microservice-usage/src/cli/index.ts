...
import { getUsageSummary } from "../lib/query.js";
...
program
  ...
  .description(
    "Usage tracking microservice — event ingestion, quota enforcement, aggregate reporting",
  )
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'usage'`;
      console.log(
        "  Schema 'usage':",
      ...
      if (schemas.length > 0) {
        const [{ count }] =
          await sql`SELECT COUNT(*) as count FROM usage.events`;
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      console.log(
        `✓ Tracked ${opts.quantity} ${opts.unit} of ${opts.metric} for workspace ${opts.workspace}`,
      );
    } finally {
      await closeDb();
    }
...
program
  ...
  .option(
    "--period <period>",
    "Period filter: day, month (affects since calculation)",
    "month",
  )
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      for (const s of summary) {
        console.log(
          `  ${s.metric}: ${s.total} ${s.unit} (since ${s.period_start})`,
        );
      }
    } finally {
      await closeDb();
    ...
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        check(
          "Schema 'usage' exists",
          schemas.length > 0,
          "run: microservice-usage migrate",
        );
        if (schemas.length > 0) {
          const [{ count }] =
            await sql`SELECT COUNT(*) as count FROM usage.events`;
