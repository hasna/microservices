...
import { getMimeType, getStorageBackend, upload } from "../lib/storage.js";
...
program
  ...
  .description(
    "Files microservice — upload, manage, and serve files with S3 and local storage",
  )
  .version("0.0.1");
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const schemas =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'files'`;
      console.log(
        "  Schema 'files':",
      ...
      if (schemas.length > 0) {
        const [{ count }] =
          await sql`SELECT COUNT(*) as count FROM files.files WHERE deleted_at IS NULL`;
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (opts.json) {
        console.log(
          JSON.stringify({ data: files, count: files.length, total }, null, 2),
        );
        return;
      }
      ...
      for (const f of files) {
        ...
        console.log(
          `  ${f.id}  ${f.name}  ${f.mime_type}  ${size}  ${f.storage}  ${f.access}`,
        );
      }
    } finally {
      await closeDb();
...
program
  ...
  .option(
    "--access <access>",
    "Access level: public, private, or signed",
    "private",
  )
  .option("--db <url>", "PostgreSQL connection URL")
  .action(async (opts) => {
    ...
    try {
      ...
      if (stats.by_mime_type.length > 0) {
        ...
        for (const entry of stats.by_mime_type) {
          ...
          console.log(
            `    ${entry.mime_type}: ${entry.count} files, ${entryMb} MB`,
          );
        }
      }
    } finally {
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log(
        "✓ microservice-files ready\n  Schema: files.*\n  Optional: S3_BUCKET (falls back to local storage)\n  Next: microservice-files serve --port 3005",
      );
    } finally {
      await closeDb();
    }
...
program
  ...
  .action(async (o) => {
    ...
    console.log(
      `  ℹ Storage: ${s3 ? `S3 (bucket: ${process.env.S3_BUCKET})` : "local (~/.hasna/files/uploads/)"}`,
    );
    if (s3) {
      check(
        "AWS_ACCESS_KEY_ID",
    ...
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        check(
          "Schema 'files' exists",
          schemas.length > 0,
          "run: microservice-files migrate",
        );
      } catch (e) {
        check(
