...
program
  ...
  .description(
    "Notify microservice — notifications, preferences, templates, webhooks",
  )
  .version("0.0.1");
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log("✓ notify schema migrations complete");
    } finally {
      await closeDb();
    }
  });
...
program
  ...
  .option("--port <n>", "Port", String(process.env.NOTIFY_PORT ?? "3004"))
  .option("--db <url>")
  .action(async (o) => {
    if (o.db) process.env.DATABASE_URL = o.db;
...
program
  .command("status")
  .description("Connection and schema status")
  .option("--db <url>")
  .action(async (o) => {
    if (o.db) process.env.DATABASE_URL = o.db;
    const sql = getDb();
    try {
      ...
      const s =
        await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'notify'`;
  ...
  .action(async (o) => {
    ...
    try {
      ...
      for (const n of items) {
        ...
        console.log(
          `  ${read} [${n.channel}] ${n.type} — ${n.title ?? n.body.slice(0, 60)}`,
        );
      }
    } finally {
      await closeDb();
...
program
  .command("test-send")
  .description("Send a test notification to verify a channel is configured")
  .requiredOption("--channel <channel>", "Channel: email|sms|in_app|webhook")
  ...
  .action(async (o) => {
    ...
    if (channel === "email") {
      ...
      try {
        ...
        console.log(
          `✗ Email delivery failed: ${e instanceof Error ? e.message : String(e)}`,
        );
        process.exit(1);
      }
    } else if (channel === "sms") {
      ...
      if (!accountSid) {
        console.log(
          "✗ SMS not configured — set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN",
        );
        process.exit(1);
      }
      ...
      console.log(
        "✓ webhook channel is always available (triggers registered endpoints)",
      );
    } else {
...
program
  ...
  .action(async (o) => {
    ...
    try {
      await migrate(sql);
      console.log(
        "✓ microservice-notify ready\n  Schema: notify.*\n  Optional: RESEND_API_KEY, TWILIO_ACCOUNT_SID\n  Next: microservice-notify serve --port 3004",
      );
    } finally {
      await closeDb();
    }
...
program
  ...
  .action(async (o) => {
    ...
    console.log(
      `  ℹ Email: ${process.env.RESEND_API_KEY ? "Resend ✓" : process.env.SMTP_HOST ? "SMTP ✓" : "not configured (in-app only)"}`,
    );
    console.log(
      `  ℹ SMS: ${process.env.TWILIO_ACCOUNT_SID ? "Twilio ✓" : "not configured"}`,
    );
    if (process.env.DATABASE_URL) {
      ...
      try {
        ...
        const schemas =
          await sql`SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'notify'`;
        check(
          "Schema 'notify' exists",
          schemas.length > 0,
          "run: microservice-notify migrate",
        );
      } catch (e) {
        check(
