...
server.tool(
  ...
  async ({ notification_id }) => {
    await recordRead(sql, notification_id);
    return text({ ok: true });
  },
);
...
server.tool(
  ...
  async ({ notification_id, metadata }) => {
    await recordClick(sql, notification_id, metadata);
    return text({ ok: true });
  },
);
...
server.tool(
  ...
  "Get all engagement events (delivery, read, click) for a notification",
  { notification_id: z.string() },
  async ({ notification_id }) =>
    text(await getNotificationEngagement(sql, notification_id)),
);

server.tool(
  "notify_channel_stats",
  "Get delivery/read/click statistics per channel for a workspace",
  {
    workspace_id: z.string(),
