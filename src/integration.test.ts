...
import { getMicroserviceStatus } from "./lib/installer.js";
...
describe("Auth — pure logic", () => {
  ...
  it("JWT sign/verify roundtrip", async () => {
    ...
    const token = await signJwt({ sub: "user-123", email: "test@test.com", type: "access" }, 60);
...
describe("Notify — template rendering", () => {
  it("renders {{variable}} substitutions", async () => {
    const notifyPath = "../microservices/microservice-notify/src/lib/templates.js";
    try {
...
describe("Full SaaS flow (PostgreSQL required)", () => {
  ...
  it("Step 1: Run all migrations", async () => {
    const authMig = await import("../microservices/microservice-auth/src/db/migrations.js");
    const teamsMig = await import("../microservices/microservice-teams/src/db/migrations.js");
    const auditMig = await import("../microservices/microservice-audit/src/db/migrations.js");
    const flagsMig = await import("../microservices/microservice-flags/src/db/migrations.js");
    const jobsMig = await import("../microservices/microservice-jobs/src/db/migrations.js");
    ...
    expect(schemas.map((s: { schema_name: string }) => s.schema_name)).toEqual(["audit", "auth", "flags", "jobs", "teams"]);
  });

  it("Step 2: Register a user (auth)", async () => {
    const { register } = await import("../microservices/microservice-auth/src/lib/auth.js");
    const result = await register(sql, { email: testEmail, password: "SecurePass123!", name: "Test User" });
  ...
  it("Step 3: Create a workspace (teams)", async () => {
    const { createWorkspace } = await import("../microservices/microservice-teams/src/lib/workspaces.js");
    const ws = await createWorkspace(sql, { name: "Test Workspace", ownerId: userId });
  ...
  it("Step 4: Verify user is owner of workspace (teams RBAC)", async () => {
    const { checkPermission } = await import("../microservices/microservice-teams/src/lib/members.js");
    ...
    expect(await checkPermission(sql, workspaceId, userId, "viewer")).toBe(true);
  });

  it("Step 5: Log audit events (audit)", async () => {
    const { logEvent, queryEvents } = await import("../microservices/microservice-audit/src/lib/events.js");

    await logEvent(sql, { actorId: userId, action: "user.registered", resourceType: "user", resourceId: userId, workspaceId });
    await logEvent(sql, { actorId: userId, action: "workspace.created", resourceType: "workspace", resourceId: workspaceId, workspaceId });
    ...
    expect(events.some((e: any) => e.action === "workspace.created")).toBe(true);
  });

  it("Step 6: Create and evaluate a feature flag (flags)", async () => {
    const { createFlag } = await import("../microservices/microservice-flags/src/lib/flags.js");
    const { evaluateFlag } = await import("../microservices/microservice-flags/src/lib/evaluate.js");

    const flag = await createFlag(sql, { key: `onboarding-${Date.now()}`, name: "Onboarding Flow", defaultValue: "true" });
  ...
  it("Step 7: Enqueue an onboarding job (jobs)", async () => {
    const { enqueue, getJob } = await import("../microservices/microservice-jobs/src/lib/queue.js");
  ...
  it("Step 8: Session is valid (auth)", async () => {
    const { getSessionByToken } = await import("../microservices/microservice-auth/src/lib/sessions.js");
    const session = await getSessionByToken(sql, sessionToken);
    expect(session?.user_id).toBe(userId);
  });

  it("Step 9: Audit log has all expected events", async () => {
    const { countEvents } = await import("../microservices/microservice-audit/src/lib/events.js");
